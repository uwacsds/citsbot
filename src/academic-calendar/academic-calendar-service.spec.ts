...
import {
  TEST_HTML_CSMARKS,
  TEST_HTML_CSMARKS_CITS1001,
  TEST_HTML_CSMARKS_CITS3200,
  TEST_HTML_WEEKS,
} from './test-data';
...
describe('academic-calendar-service', () => {
  ...
  nock(TEACHING_WEEKS_BASE).get(TEACHING_WEEKS_PATH).reply(200, TEST_HTML_WEEKS);
  nock(CS_MARKS_BASE).get(CS_MARKS_PATH).reply(200, TEST_HTML_CSMARKS);
  nock(CS_MARKS_BASE).get(CS_MARKS_UNIT_PATH).times(999).reply(200, TEST_HTML_CSMARKS_CITS1001);
