...
const fetchChannel = (client: Client) => async (id: string): Promise<Channel | null> => client.channels.fetch(id);

const fetchTextChannel = (client: Client) => async (id: string): Promise<TextChannel | null> => {
  const channel = await fetchChannel(client)(id);
  if (channel?.type !== 'text') return null;
  return channel as TextChannel;
};

const fetchMessage = (client: Client) => async (channelId: string, messageId: string): Promise<Message | null> => {
