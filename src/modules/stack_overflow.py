...
class StackOverflow(commands.Cog):
    ...
    async def stack_overflow(self, ctx, *, search_terms):
        try:
            ...
            embedded_msg = embed.Embed(title=f"Q: {question_title}", 
                                        url=question_url, 
                                        description=answer_body,  
                                        colour=colour.Colour.from_rgb(244, 128, 36))
            embedded_msg.set_author(name="Stack Overflow", icon_url="https://i.imgur.com/d6LbAzp.png")
            await ctx.channel.send(embed=embedded_msg)
        except:
            await ctx.channel.send("Failed to find a stack overflow question matching those terms")
