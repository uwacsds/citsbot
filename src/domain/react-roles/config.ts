...
export const validateReactRolesConfig = (rawConfig: Record<string, unknown>, unitsConfig: Record<string, UnitConfig>): ReactRolesConfig => {
  ...
  return rawConfig as ReactRolesConfig;
};
...
const validateReactionConfig = (unitsConfig: Record<string, UnitConfig>, messageIdx: number) => (rawConfig: Record<string, unknown>, reactIdx: number): rawConfig is ReactRolesMessageReactionConfig => {
  ...
  if (!hasUnit && !hasRole) throw Error(`${prefix}: Reactions must have exactly one of either 'role' or 'unit'`);
