...
describe('command-handler', () => {
  ...
  it('should react to a welcome message', async () => {
    const actions = await Promise.all(
      onMessage({ content: 'welcome foo!', channel: { id: config.modules.welcomer.channel }, id: 'msg1' } as any)
    );
    expect(filterNoActions(actions)).toEqual([
      {
  ...
  it('should send a welcome message when a member joins', async () => {
    ...
    expect(filterNoActions(actions)).toMatchObject([
  ...
  it('should grant a role when a user reacts', async () => {
    ...
    expect(filterNoActions(actions)).toMatchObject([
      { type: BotActionType.RoleGrant, guild: 'guild_1', user, role: 'role1' },
    ]);
  });
  ...
  it('should revoke a role when a user reacts', async () => {
    ...
    expect(filterNoActions(actions)).toMatchObject([
      { type: BotActionType.RoleRevoke, guild: 'guild_1', user, role: 'role1' },
    ]);
  });
});
