...
describe(`anime detector config`, () => {
  ...
  test(`given invalid keywordCountThreshold > when validate > should throw`, () => {
    ...
    expect(() => validateAnimeDetectorConfig(rawConfig)).toThrowError(`Config Validation: modules.animeDetector: Expected 'keywordCountThreshold' to have type 'number', got 'undefined'`);
  });

  test(`given invalid keywords > when validate > should throw`, () => {
    const rawConfig = { ...validConfig, keywords: undefined };
    expect(() => validateAnimeDetectorConfig(rawConfig)).toThrowError(`Config Validation: modules.animeDetector: Expected 'keywords' to have type 'array', got 'undefined'`);
  });

  test(`given invalid keyword type > when validate > should throw`, () => {
    const rawConfig = { ...validConfig, keywords: [...validConfig.keywords, 123] };
    expect(() => validateAnimeDetectorConfig(rawConfig)).toThrowError(`Config Validation: modules.animeDetector.keywords[3]: Expected element to have type 'string', got 'number'`);
  });
});
