...
describe('cowsay-module', () => {
  ...
  const message: DiscordMessage = {
    id: '1', createdAt: now(), deletable: true, content: '',
  ...
  it('should not do anything on a standard message with no prefix', async () => {
    await expect(cowsay.onMessage({ ...message, content: 'hello world' })).resolves.toEqual({ type: BotActionType.Nothing });
  });

  it('should not do anything with a different prefix', async () => {
    await expect(cowsay.onMessage({ ...message, content: '!announce now' })).resolves.toEqual({ type: BotActionType.Nothing });
  });

  it('should not do anything when no space after prefix', async () => {
    await expect(cowsay.onMessage({ ...message, content: '!cowsayno baa' })).resolves.toEqual({ type: BotActionType.Nothing });
  });
});
