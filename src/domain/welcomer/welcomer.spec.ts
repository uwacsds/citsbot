...
describe('welcomer module', () => {
  ...
  const config: WelcomerConfig = { channel: 'channelId', newMemberDm: { delay: 1, instantAccountAge: 1, message: '', react: 'reactEmoji', roleThreshold: 1 } };
  ...
  it('should create an embedded message action to welcome the user', async () => {
    ...
    await expect(welcomer.onMemberJoin(user)).resolves.toEqual({
      ...
      embed: {
        title: 'Hello, world!', description: `Hey, ${user.username}`, colour: '#0864a5', thumbnail: user.avatar,
  ...
  it('should create a wave at event for a message', async () => {
    ...
    await expect(welcomer.onMessage(message)).resolves.toEqual({ type: BotActionType.AddReaction, channelId: message.channel.id, messageId: message.id, emoji: config.newMemberDm.react });
  });
});
